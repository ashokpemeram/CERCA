...
void main() {
  group('Admin simulation flow', () {
    test('start and stop use backend simulation endpoints and apply returned assessment', () async {
        ...
        if (request.url.path.endsWith('/simulation/start')) {
          ...
          return http.Response(
            jsonEncode({
              ...
              'area': {
                ..._area(redRadiusM: 1800, warningRadiusM: 2200, greenRadiusM: 2600, controllableRadiusM: 3200)
                    .toJson(),
              },
              'assessment': {
    ...
    test('restoreSimulationForCurrentArea rehydrates backend session', () async {
      final apiClient = MockClient((request) async {
        if (request.url.path.endsWith('/simulation/active')) {
          return http.Response(
            jsonEncode({
